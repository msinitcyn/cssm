...
@patch("aws_scanner.scanners.scan_orchestrator.CloudFormationReader")
def test_cloudformation_path(mock_cf_reader, mock_analyze, mock_format, mock_generate):
    mock_collection = ResourceCollection()
    mock_reader_instance = Mock()
    mock_reader_instance.read.return_value = mock_collection
    mock_cf_reader.return_value = mock_reader_instance
    ...
    mock_cf_reader.assert_called_once_with("/path/to/template.yaml")
...
def test_iam_policy_aws_api_raises_not_implemented():
    config = RunConfig(
        cloudformation=None,
        s3=None,
        iam_role=None,
        iam_policy=IamPolicyConfig(file=None, attached_only=False),
        sg=None,
        report=ReportConfig(path=Path("output/report.json"))
    )

    with pytest.raises(NotImplementedError, match="AWS API scanning not yet migrated to resource path"):
        run_scan(config)


def test_iam_role_aws_api_raises_not_implemented():
    config = RunConfig(
        cloudformation=None,
        s3=None,
...
def test_s3_aws_api_raises_not_implemented():
    config = RunConfig(
        cloudformation=None,
...
def test_sg_aws_api_raises_not_implemented():
    config = RunConfig(
        cloudformation=None,
        s3=None,
        iam_role=None,
        iam_policy=None,
        sg=SgConfig(file=None, regions=["us-east-1"]),
        report=ReportConfig(path=Path("output/report.json"))
    )
